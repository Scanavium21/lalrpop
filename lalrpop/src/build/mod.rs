...
fn process_file_into(
    ...
    if session.force_build || needs_rebuild(lalrpop_file, rs_file)? {
        ...
        // file behind.
        {
            ...
            output_file.write_all(&ast_buffer)?;
    ...
    #[cfg(not(feature = "ast-gen"))]
    {
        for item in &grammar.items {
            let nonterminal = match item {
                pt::GrammarItem::Nonterminal(nonterminal) => nonterminal,
                _ => continue,
            };
            let type_ref = match &nonterminal.type_decl {
...
fn normalize_grammar(session: &Session, file_text: &FileText, grammar: &mut pt::Grammar) -> io::Result<r::Grammar> {
